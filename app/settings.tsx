...
import { View, Text, TouchableOpacity, StyleSheet, ScrollView, Switch, TextInput, Alert, Share } from 'react-native';
...
import { persistLog, getPersistedLogs, clearPersistedLogs, LogCategory, LOG_COLORS, LOG_LABELS } from '../lib/logs';
    ...
    <View style={[styles.container, { backgroundColor: C.background }]}>
      ...
      <ScrollView style={styles.content} contentContainerStyle={styles.contentInner}>
        ...
        <View style={[styles.card, { backgroundColor: C.card, borderColor: C.border }]}>
          ...
          <View style={[styles.card, { backgroundColor: C.card, borderColor: C.border }]}>
            <View style={[styles.sectionHeader, { borderBottomColor: C.border }]}>
              ...
              <Text style={[styles.sectionTitle, { color: C.foreground }]}>Full-Screen Mode</Text>
            </View>
            <View style={[styles.settingRow]}>
              <View style={{ flex: 1 }}>
                <Text style={[styles.settingLabel, { color: C.foreground }]}>Full-Screen Mode (APK)</Text>
                <Text style={[styles.settingDesc, { color: C.mutedForeground }]}>
                  Hide status bar for immersive testing experience
                </Text>
              </View>
              <TouchableOpacity
                style={[styles.customToggle, fullscreenEnabled ? { backgroundColor: '#10B981' } : { backgroundColor: C.muted, borderColor: C.border }]}
                onPress={async () => {
                  const newVal = !fullscreenEnabled;
                  setFullscreenEnabled(newVal);
                  await setItem('examforge_fullscreen_mode', newVal ? 'true' : 'false');
                  logAction(`Full-screen mode: ${newVal ? 'ON' : 'OFF'}`, newVal ? '#10B981' : '#F59E0B');
                }}
              >
                <View style={[styles.customToggleDot, fullscreenEnabled ? { transform: [{ translateX: 16 }] } : {}]} />
              </TouchableOpacity>
            </View>
          </View>
                    ...
                    <View style={{
                      ...
                      <TextInput
                        ...
                        onChangeText={(t) => {
                          ...
                          if (!isNaN(v) && v >= 1 && v <= 200) setAiCount(v);
                        }}
                      />
                    </View>
                  </View>
              ...
              <View>
                ...
                <View style={[styles.countSliderContainer, { backgroundColor: C.muted, borderColor: C.border }]}>
                  ...
                  <View style={{ position: 'relative', height: 6, backgroundColor: C.border, borderRadius: 3 }}>
                    <View style={{
                      position: 'absolute', left: 0, top: 0, height: 6, borderRadius: 3,
                      backgroundColor: '#8B5CF6', width: `${((manualCount - 1) / 199) * 100}%`,
                    }} />
                  </View>
                  {/* Quick preset chips */}
                  <View style={{ flexDirection: 'row', gap: Spacing.xs, flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <View style={{
                      ...
                      <TextInput
                        style={{ fontSize: FontSize.sm, fontWeight: '600', color: C.foreground, textAlign: 'center', minWidth: 40, padding: 0 }}
                        value={[5, 10, 20, 30, 50, 75, 100, 150, 200].includes(manualCount) ? '' : String(manualCount)}
                        placeholder="Any"
                        placeholderTextColor={C.mutedForeground}
                        keyboardType="number-pad"
                        onChangeText={(t) => {
                          const v = parseInt(t, 10);
                          if (!isNaN(v) && v >= 1 && v <= 200) { setManualCount(v); logAction(`Manual count custom: ${v}`, '#8B5CF6'); }
                        }}
                      />
                    </View>
                  </View>
        ...
        <View style={[styles.card, { backgroundColor: C.card, borderColor: C.border }]}>
          ...
          >
            <View style={{ flex: 1 }}>
              <Text style={[styles.settingLabel, { color: C.foreground }]}>Storage</Text>
              <Text style={[styles.settingDesc, { color: C.mutedForeground }]}>
                View storage usage, backup and restore data
              </Text>
            </View>
            <Ionicons name="chevron-forward" size={18} color={C.mutedForeground} />
          </TouchableOpacity>
        ...
        <View style={[styles.card, { backgroundColor: C.card, borderColor: C.border }]}>
          ...
          <View style={styles.aboutRow}>
            ...
            <Text style={[styles.aboutValue, { color: C.foreground }]}>1.2</Text>
          </View>
          <View style={[styles.divider, { backgroundColor: C.border }]} />
          <View style={styles.aboutRow}>
            <Text style={[styles.aboutLabel, { color: C.mutedForeground }]}>App</Text>
            <Text style={[styles.aboutValue, { color: C.foreground }]}>Axam</Text>
          </View>
          <View style={[styles.divider, { backgroundColor: C.border }]} />
          <View style={styles.aboutRow}>
        ...
        <View style={[styles.card, { backgroundColor: C.card, borderColor: C.border }]}>
          <TouchableOpacity style={[styles.sectionHeader, { borderBottomColor: C.border }]} onPress={() => setShowLogs(!showLogs)}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: Spacing.md, flex: 1 }}>
              ...
              <Text style={[styles.sectionTitle, { color: C.foreground }]}>Application Logs</Text>
            </View>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: Spacing.sm }}>
              {appLogs.length > 0 && (
                <TouchableOpacity onPress={async () => { await clearPersistedLogs(); setAppLogs([]); }}>
                ...
                {appLogs.length === 0 ? (
                  <View style={{ padding: Spacing.md, alignItems: 'center' }}>
                    <Text style={[styles.terminalText, { color: '#71717A', textAlign: 'center' }]}>
                      No logs yet. Perform actions in the app to see logs here.
                    </Text>
                  </View>
                ) : (
                  appLogs.map((log, i) => (
                    <View key={i} style={styles.terminalLine}>
                      ...
                      <Text style={[styles.terminalText, { color: log.color || '#D4D4D8' }]}>
                        {log.category && <Text style={{ color: LOG_COLORS[log.category] || log.color, fontWeight: '700', fontSize: 9, letterSpacing: 1 }}>[{LOG_LABELS[log.category]}] </Text>}{log.screen ? <Text style={{ color: '#6366F1', fontWeight: '700' }}>[{log.screen.toUpperCase()}] </Text> : null}{log.text}
                      </Text>
                    </View>
                  ))
                )}
              ...
              {appLogs.length > 0 && (
                <View style={[styles.terminalFooter, { borderTopColor: '#27272A' }]}>
                  <TouchableOpacity
                    ...
                    onPress={async () => {
                      try {
                        const logText = appLogs.map(l => '[' + l.time + '] [' + (l.screen || "app") + '] ' + l.text).join('\n');
                        const logFile = new File(Paths.document, 'examforge-logs.txt');
                        await logFile.write(logText || 'No logs');
                        const uri = logFile.uri;
                        
                        await Share.share({ title: 'Axam Logs', message: logText || 'No logs', url: uri });
                      } catch(e) {}
                    }}>
                    <Ionicons name="share-outline" size={12} color="#6366F1" />
                    <Text style={{ fontSize: 10, color: '#6366F1', fontWeight: '600', marginLeft: 4 }}>Export</Text>
                  </TouchableOpacity>
                  <Text style={{ fontSize: 10, color: '#71717A' }}>
                    {appLogs.length} log entries
                  </Text>
                </View>
              )}
            </View>
        ...
        <View style={[styles.notice, { backgroundColor: C.primary + '08', borderColor: C.primary + '20' }]}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={[styles.noticeDesc, { color: C.mutedForeground }]}>
              ...
              Axam never transmits these credentials to our servers.
            </Text>
          </View>
        </View>
