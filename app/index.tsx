...
export default function HomeScreen() {
  ...
  const logAction = (text: string, color?: string) => {
    setProcessLogs(prev => addLog(prev, text, color));
  ...
  const hasApiKey = aiProvider === 'gemini' ? !!geminiApiKey : !!apiKey;
  ...
  const handleManualGenerate = () => {
    ...
    try {
      ...
      const maxCount = Math.min(manualCount, 100);
  ...
  const handleStartTest = async (config: TestConfig) => {
    ...
    logAction(`Provider: ${latestProvider}, Model: ${latestProvider === 'gemini' ? latestGModel : latestORModel}`, C.warning);
    ...
    const enhancedConfig: TestConfig = {
      ...config,
      aiModel: latestProvider === 'gemini' ? latestGModel : latestORModel,
    ...
    try {
      ...
      const result = await generateTest(
        ...
        latestApiKey, latestGeminiKey, storedQuestions, allStored.length,
      );
      ...
      if (error.message === 'GEMINI_QUOTA_EXCEEDED' && latestApiKey) {
        try {
          ...
          const fallbackResult = await generateTest(
            ...
            latestApiKey, latestGeminiKey, storedQuestions, activeSection.storedQuestions.length,
          );
  ...
  const handleQuizComplete = async (userAnswers: Record<string, number>, time: number) => {
    ...
    if (retakingTestId) {
      ...
      if (existing) {
        await updateTestResultInSection(activeSectionId, retakingTestId, {
          ...existing, date: new Date().toISOString(), answers: userAnswers, score: correctCount, timeTaken: time,
        });
          ...
          <>
            ...
            <View style={[styles.aiTestCard, { backgroundColor: C.card, borderColor: C.border }]}>
              <TouchableOpacity
                ...
                onPress={() => { setShowProcessLogs(!showProcessLogs); logAction(showProcessLogs ? 'Process logs hidden' : 'Process logs shown', '#1E293B'); }}
              >
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: Spacing.md, flex: 1 }}>
                  <View style={[styles.aiIconBox, { backgroundColor: '#1E293B' + '15' }]}>
                    <Ionicons name="terminal-outline" size={20} color="#1E293B" />
                  </View>
                  <Text style={[styles.aiSectionTitle, { color: C.foreground }]} numberOfLines={1}>Process Logs</Text>
                </View>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: Spacing.sm }}>
                  {processLogs.length > 0 && (
                    <TouchableOpacity onPress={() => setProcessLogs([])}>
                      <Text style={{ fontSize: 10, color: C.destructive, fontWeight: '600' }}>Clear</Text>
                    </TouchableOpacity>
                  )}
                  <Ionicons name={showProcessLogs ? "chevron-up" : "chevron-down"} size={20} color={C.mutedForeground} />
                </View>
              </TouchableOpacity>
              {showProcessLogs && (
                <View style={{ padding: Spacing.md }}>
                  <View style={[styles.terminalLogBox, { backgroundColor: '#09090B', borderColor: '#27272A' }]}>
                    <ScrollView style={{ maxHeight: 180 }} nestedScrollEnabled>
                      {processLogs.length === 0 ? (
                        <View style={{ padding: Spacing.md }}>
                          <Text style={{ fontSize: 11, color: '#71717A', fontFamily: 'monospace' }}>
                            No process logs yet. Generate a test to see logs here.
                          </Text>
                        </View>
                      ) : (
                        processLogs.map((log, i) => (
                          <View key={i} style={{ flexDirection: 'row', gap: Spacing.sm, marginBottom: 6 }}>
                            <Text style={{ fontSize: 10, fontWeight: '700', color: '#71717A', fontFamily: 'monospace' }}>{log.time}</Text>
                            <Text style={{ fontSize: 11, flex: 1, color: log.color || '#D4D4D8', fontFamily: 'monospace' }}>{log.text}</Text>
                          </View>
                        ))
                      )}
                    </ScrollView>
                    {processLogs.length > 0 && (
                      <View style={{ flexDirection: 'row', justifyContent: 'flex-end', paddingTop: Spacing.sm, marginTop: Spacing.sm, borderTopWidth: 1, borderTopColor: '#27272A' }}>
                        <Text style={{ fontSize: 10, color: '#71717A' }}>{processLogs.length} log entries</Text>
                      </View>
                    )}
                  </View>
                </View>
...
const styles = StyleSheet.create({
  ...
  statCard: { borderRadius: BorderRadius.md, borderWidth: 1, padding: Spacing.md, alignItems: 'center', justifyContent: 'center', gap: Spacing.xs },
  ...
  aiTestCard: { borderRadius: BorderRadius.lg, borderWidth: 1 },
  sectionHeaderTest: { flexDirection: 'row', alignItems: 'center', padding: Spacing.lg, borderBottomWidth: 1, flexWrap: 'wrap', gap: Spacing.sm },
  aiIconBox: { width: 40, height: 40, borderRadius: BorderRadius.md, alignItems: 'center', justifyContent: 'center' },
  aiSectionTitle: { fontSize: FontSize.lg, fontWeight: '600' },
  ...
  generateBtn: { paddingVertical: Spacing.lg, borderRadius: BorderRadius.md, alignItems: 'center', justifyContent: 'center', flexDirection: 'row' },
  ...
  processLogsCard: { borderRadius: BorderRadius.lg, borderWidth: 1 },
  processLogsHeader: { flexDirection: 'row', alignItems: 'center', padding: Spacing.lg, borderBottomWidth: 1 },
  terminalLogBox: { borderRadius: BorderRadius.xl, padding: Spacing.md, borderWidth: 1 },
});
