...
function countJsonQuestions(json: string): number | null {
  try {
    const p = JSON.parse(json);
    if (p.questions && Array.isArray(p.questions)) return p.questions.length;
    if (Array.isArray(p)) return p.length;
    return null;
  } catch { return null; }
}

type AppView = 'config' | 'quiz' | 'results' | 'history' | 'similarity' | 'shuffle';
export default function HomeScreen() {
  ...
  const [shuffleTitle, setShuffleTitle] = useState('Test');
          ...
          if (section) {
            ...
            if (test) {
              ...
              setTimeout(() => {
                ...
                setShuffleTitle(test.name || 'Retake Test');
                setView('shuffle');
  ...
  const handleManualGenerate = () => {
    ...
    if (!activeSectionId) {
      Alert.alert('No Section', 'Create or select a section first.');
      return;
    }
    ...
    try {
      ...
      // Accept all questions from JSON — settings questionCount only affects Copy Prompt, not paste
      const validated: Question[] = questionsData.map((q: any, i: number) => ({
  ...
  const handleStartTest = async (config: TestConfig) => {
    ...
    try {
      ...
      if (storedQuestions.length > 0) {
        ...
        logAction('No stored questions to compare — opening shuffle screen.', '#10B981');
        setShuffleTitle('AI Generated Test');
        setView('shuffle');
      }
    } catch (error: any) {
  ...
  const handleStartQuizFromSimilarity = () => { setShuffleTitle('AI Generated Test'); setView('shuffle'); }
  ...
  const handleRetry = () => {
    setShuffleTitle('Retry Test');
    setView('shuffle');
    logAction('Opening shuffle screen for retry...', '#10B981');
  };

  const handleShuffleConfirm = (shuffledQuestions: Question[]) => {
    setQuestions(shuffledQuestions);
    setView('quiz');
    logAction(`Shuffle confirmed: ${shuffledQuestions.length} questions ready.`, '#10B981');
  };

  const handleShuffleCancel = () => {
    setRetakingTestId(null);
    setView('config');
    logAction('Shuffle cancelled.', '#F59E0B');
  };
  const handleNewTest = () => {
  ...
  const handleManualTestGenerate = (questions: Question[], config: TestConfig) => {
    ...
    if (section && section.storedQuestions.length > 0) {
      setView('similarity');
    } else {
      setShuffleTitle('Manual Test');
      setView('shuffle');
    }
  };
                ...
                <View style={{ gap: Spacing.md, padding: Spacing.md }}>
                  ...
                  <View>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: Spacing.sm }}>
                      ...
                      <Text style={[styles.fieldLabel, { color: C.mutedForeground, marginBottom: 0, flex: 1 }]}>Paste AI JSON Response</Text>
                      <TouchableOpacity
                        style={{ flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#8B5CF6' + '18', paddingHorizontal: 12, paddingVertical: 6, borderRadius: BorderRadius.sm, borderWidth: 1, borderColor: '#8B5CF6' + '40' }}
                        onPress={async () => {
                          try {
                            const text = await Clipboard.getStringAsync();
                            if (text && text.trim()) {
                              setPastedManualJson(text.trim());
                              setManualParseError(null);
                              logAction('JSON pasted from clipboard.', '#10B981');
                            } else {
                              setManualParseError('Clipboard is empty. Copy JSON first, then tap Paste.');
                            }
                          } catch (e: any) {
                            setManualParseError('Clipboard error: ' + (e?.message || 'unknown'));
                          }
                        }}
                        activeOpacity={0.6}
                      >
                        <Ionicons name="clipboard-outline" size={14} color="#8B5CF6" />
                        <Text style={{ fontSize: 12, fontWeight: '600', color: '#8B5CF6' }}>Paste</Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                        style={{ flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#F43F5E' + '18', paddingHorizontal: 12, paddingVertical: 6, borderRadius: BorderRadius.sm, borderWidth: 1, borderColor: '#F43F5E' + '40' }}
                        onPress={() => { setPastedManualJson(''); setManualParseError(null); }}
                        activeOpacity={0.6}
                      >
                        <Ionicons name="trash-outline" size={14} color="#F43F5E" />
                        <Text style={{ fontSize: 12, fontWeight: '600', color: '#F43F5E' }}>Clear</Text>
                      </TouchableOpacity>
                    </View>
                    <TextInput
                      style={[styles.aiTextArea, { minHeight: 80, backgroundColor: C.background, borderColor: manualParseError ? '#F43F5E' : C.border, color: C.foreground }]}
                      multiline
                      placeholder='{"questions": [...]}'
                      placeholderTextColor={C.mutedForeground + '60'}
                      value={pastedManualJson}
                      onChangeText={(t) => { setPastedManualJson(t); if (manualParseError) setManualParseError(null); }}
                      textAlignVertical="top"
                      autoCapitalize="none"
                      autoCorrect={false}
                    />
                  </View>
                  {/* Question count or error */}
                  {pastedManualJson.trim() ? (
                    <View style={{
                      flexDirection: 'row', alignItems: 'center', gap: 8, padding: Spacing.md,
                      backgroundColor: countJsonQuestions(pastedManualJson) !== null ? '#10B981' + '10' : '#F43F5E' + '10',
                      borderRadius: BorderRadius.md, borderWidth: 1,
                      borderColor: countJsonQuestions(pastedManualJson) !== null ? '#10B981' + '25' : '#F43F5E' + '30',
                    }}>
                      <Ionicons
                        name={countJsonQuestions(pastedManualJson) !== null ? 'checkmark-circle' : 'alert-circle'}
                        size={20}
                        color={countJsonQuestions(pastedManualJson) !== null ? '#10B981' : '#F43F5E'}
                      />
                      <View style={{ flex: 1 }}>
                        {countJsonQuestions(pastedManualJson) !== null ? (
                          <>
                            <Text style={{ fontSize: FontSize.md, color: '#10B981', fontWeight: '700' }}>{countJsonQuestions(pastedManualJson)} Questions Found</Text>
                            <Text style={{ fontSize: 10, color: '#10B981' + 'AA' }}>Valid JSON · Ready to generate</Text>
                          </>
                        ) : (
                          <>
                            <Text style={{ fontSize: FontSize.md, color: '#F43F5E', fontWeight: '700' }}>Invalid JSON</Text>
                            <Text style={{ fontSize: 10, color: '#F43F5E' + 'AA' }}>Must contain a "questions" array or be a questions array</Text>
                          </>
                        )}
                      </View>
                    </View>
                  ) : manualParseError ? (
                    <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 8, padding: Spacing.md, backgroundColor: '#F43F5E' + '10', borderRadius: BorderRadius.md, borderWidth: 1, borderColor: '#F43F5E' + '30' }}>
                      <Ionicons name="alert-circle" size={18} color="#F43F5E" style={{ marginTop: 2 }} />
                      <Text style={{ fontSize: FontSize.sm, color: '#F43F5E', flex: 1, lineHeight: 20 }}>{manualParseError}</Text>
                    </View>
                  ) : null}
                  {/* Generate Test Button */}
                  <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  clipseBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 5, borderRadius: BorderRadius.md, borderWidth: 1 },
