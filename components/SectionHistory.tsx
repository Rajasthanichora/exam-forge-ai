    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        ...
        <View style={styles.quickStatsGrid}>
          <View style={[styles.quickStat, { backgroundColor: C.card, borderColor: C.border }]}>
            <View style={styles.quickStatTop}>
              <View style={[styles.quickStatIcon, { backgroundColor: '#10B981' + '12' }]}>
                <Ionicons name="checkmark-circle" size={22} color="#10B981" />
              </View>
              {totalTests > 0 && (
                <Text style={[styles.quickStatBadge, { backgroundColor: avgScore >= 80 ? '#10B981' + '12' : avgScore >= 60 ? C.primary + '12' : C.warning + '12', borderColor: avgScore >= 80 ? '#10B981' + '30' : avgScore >= 60 ? C.primary + '30' : C.warning + '30', color: avgScore >= 80 ? '#10B981' : avgScore >= 60 ? C.primary : C.warning }]}>
                  {getGradeLabel(avgScore)}
                </Text>
              )}
            </View>
            <Text style={[styles.quickStatValue, { color: C.foreground }]}>
              {totalTests > 0 ? `${totalCorrectAnswers} / ${totalTests}` : '0'}
            </Text>
          ...
          <View style={[styles.quickStat, { backgroundColor: C.card, borderColor: C.border }]}>
            <View style={styles.quickStatTop}>
              <View style={[styles.quickStatIcon, { backgroundColor: C.primary + '12' }]}>
                <Ionicons name="time-outline" size={22} color={C.primary} />
              </View>
            </View>
            <Text style={[styles.quickStatValue, { color: C.foreground }]}>
              {testResults.length > 0 ? formatTime(totalTimeTaken) : '0m 0s'}
            </Text>
            <Text style={[styles.quickStatLabel, { color: C.mutedForeground }]}>Completion Time</Text>
          </View>

          <View style={[styles.quickStat, { backgroundColor: C.card, borderColor: C.border }]}>
            <View style={styles.quickStatTop}>
              <View style={[styles.quickStatIcon, { backgroundColor: C.warning + '12' }]}>
                <Ionicons name="bar-chart-outline" size={22} color={C.warning} />
              </View>
            </View>
            <Text style={[styles.quickStatValue, { color: C.foreground }]}>{totalTests > 0 ? getGradeLabel(avgScore) : '—'}</Text>
            <Text style={[styles.quickStatLabel, { color: C.mutedForeground }]}>Performance Grade</Text>
          </View>

          <View style={[styles.quickStat, { backgroundColor: C.card, borderColor: C.border }]}>
            <View style={styles.quickStatTop}>
              <View style={[styles.quickStatIcon, { backgroundColor: C.muted }]}>
                <Ionicons name="trophy-outline" size={22} color={C.mutedForeground} />
              </View>
            </View>
            <Text style={[styles.quickStatValue, { color: C.foreground }]}>
              {avgScore >= 90 ? 'Gold Tier' : avgScore >= 70 ? 'Silver Tier' : 'Bronze Tier'}
            </Text>
            <Text style={[styles.quickStatLabel, { color: C.mutedForeground }]}>Certification Badge</Text>
          </View>
        </View>
      </View>
          ...
          <View key={result.id} style={[styles.reviewItem, {
            ...
            <View style={styles.reviewContent}>
              <View style={[styles.reviewNum, {
                ...
                <Text style={{ fontWeight: '800', fontSize: FontSize.sm, color: isCorrectItem ? '#10B981' : C.destructive }}>
                  {idx + 1}
                </Text>
              </View>
              <View style={{ flex: 1 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: Spacing.sm }}>
                  <Text style={[styles.reviewQuestion, { color: C.foreground }]}>
                    {result.name}
                  </Text>
                  <View style={[styles.reviewBadge, {
                    backgroundColor: isCorrectItem ? '#10B981' : C.destructive,
                  }]}>
                    <Text style={styles.reviewBadgeText}>
                      {isCorrectItem ? 'Correct' : 'Incorrect'}
                    </Text>
                  </View>
                </View>
                ...
                <View style={[styles.reviewStats, { borderTopColor: C.border }]}>
                  <Text style={{ fontSize: FontSize.xs, color: C.mutedForeground }}>
                    Score: <Text style={{ fontWeight: '700', color: getScoreColor(pct) }}>{pct}%</Text>
                  </Text>
                  <Text style={{ fontSize: FontSize.xs, color: C.mutedForeground }}>
      ...
      <View style={[styles.historySection, { borderTopColor: C.border }]}>
        <View style={styles.historyHeader}>
          <Text style={[styles.sectionTitle, { color: C.foreground }]}>Attempt History</Text>
        ...
        {testResults.length > 0 && (
          <TouchableOpacity style={{ alignSelf: 'flex-end', marginBottom: Spacing.sm }}
            onPress={() => {
              Alert.alert('Clear All History?', 'This will delete all test history and tracking data.', [
                { text: 'Cancel', style: 'cancel' },
                { text: 'Clear All', style: 'destructive', onPress: onClearAll },
              ]);
            }}>
            <Text style={{ fontSize: 11, color: C.destructive, fontWeight: '600' }}>
              <Ionicons name="trash-outline" size={12} color={C.destructive} />  Clear All History
            </Text>
          </TouchableOpacity>
        )}
        ...
        {filteredResults.length === 0 ? (
          ...
          <View style={[styles.tableCard, { backgroundColor: C.card, borderColor: C.border }]}>
            {/* Table Header */}
            <View style={[styles.tableHeader, { backgroundColor: isDark ? C.muted : '#F8FAFC', borderBottomColor: C.border }]}>
              <Text style={[styles.th, { color: C.mutedForeground }, { flex: 2.5 }]}>Assessment Name</Text>
              <Text style={[styles.th, { color: C.mutedForeground }]}>Date</Text>
              <Text style={[styles.th, { color: C.mutedForeground }]}>Score</Text>
              <Text style={[styles.th, { color: C.mutedForeground }]}>Difficulty</Text>
              <Text style={[styles.th, { color: C.mutedForeground }, styles.thRight]}>Actions</Text>
            </View>
            ...
            {filteredResults.map(result => {
              ...
              return (
                <View key={result.id} style={[styles.tableRow, { borderBottomColor: isDark ? C.border : '#F1F5F9' }]}>
                    ...
                    {editingId === result.id ? (
                      <View style={styles.editRow}>
                        <TextInput
                          ...
                          autoFocus
                          onSubmitEditing={() => {
                            if (editName.trim()) { onRenameTest(result.id, editName.trim()); setEditingId(null); }
                          }}
                        />
                      ...
                      <>
                        <Text style={[styles.testName, { color: C.foreground }]}>{result.name}</Text>
                        <Text style={[styles.testMeta, { color: C.mutedForeground }]}>
                          {result.totalQuestions} Items  •  {result.config.language}
                        </Text>
                      </>
                    )}
                  </View>
                  <Text style={[styles.cell, { color: C.mutedForeground }, styles.dateCell]}>{formatDate(result.date)}</Text>
                  <View style={styles.cell}>
                    <View style={[styles.scoreBadge, { backgroundColor: scoreColor + '12', borderColor: scoreColor + '30' }]}>
                      <Text style={[styles.scoreBadgeText, { color: scoreColor }]}>{pct}%</Text>
                    </View>
                  </View>
                  <Text style={[styles.cell, { color: C.mutedForeground }, styles.diffCell]}>{result.config.difficulty}</Text>
                  <View style={[styles.cell, styles.actionsCell]}>
                    {editingId !== result.id && (
                      <View style={{ flexDirection: 'row', gap: Spacing.xs }}>
                        <TouchableOpacity onPress={() => { setEditingId(result.id); setEditName(result.name); }}
...
const styles = StyleSheet.create({
  ...
  headerBtnText: { fontSize: 10, fontWeight: '700', letterSpacing: 0.5, flexShrink: 1 },
  ...
  quickStat: {
    width: '48%', borderRadius: BorderRadius.xl, borderWidth: 1, alignItems: 'center',
    padding: Spacing.lg, gap: Spacing.xs,
  },
  quickStatTop: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginBottom: Spacing.md },
  quickStatIcon: { width: 44, height: 44, borderRadius: BorderRadius.md, alignItems: 'center', justifyContent: 'center' },
  quickStatBadge: { fontSize: 10, fontWeight: '800', paddingHorizontal: Spacing.sm, paddingVertical: 2, borderRadius: Spacing.xs, borderWidth: 1, overflow: 'hidden' },
  quickStatValue: { fontSize: 24, fontWeight: '900', letterSpacing: -0.5, textAlign: 'center' },
  quickStatLabel: { fontSize: 10, fontWeight: '700', letterSpacing: 0.5, textAlign: 'center' },
  reviewSection: {
  ...
  reviewBadgeText: { fontSize: 9, fontWeight: '800', color: '#FFFFFF', letterSpacing: 0.5 },
