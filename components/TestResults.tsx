...
export default function TestResults({
  ...
  const filteredQuestions = reviewFilter === 'all' ? questions : questions.filter(q => {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={[styles.card, { backgroundColor: C.card, borderColor: C.border }]}>
        ...
        {/* Filter buttons for All, Correct, Incorrect */}
          ...
          {(['all', 'correct', 'incorrect'] as const).map(f => (
            <TouchableOpacity
              ...
              style={{
                ...
                borderColor: reviewFilter === f ? (f === 'correct' ? '#10B981' : f === 'incorrect' ? C.destructive : C.primary) : C.border,
                backgroundColor: reviewFilter === f ? (f === 'correct' ? '#10B981' + '15' : f === 'incorrect' ? C.destructive + '15' : C.primary + '15') : 'transparent',
              }}
              onPress={() => setReviewFilter(f)}
            >
              <Text style={{
                fontSize: FontSize.sm, fontWeight: '600', textTransform: 'capitalize',
                color: reviewFilter === f ? (f === 'correct' ? '#10B981' : f === 'incorrect' ? C.destructive : C.primary) : C.mutedForeground,
              }}>
        ...
        {filteredQuestions.length === 0 && (
          <View style={{ padding: Spacing.lg, alignItems: "center" }}>
            <Text style={{ fontSize: FontSize.md, color: C.mutedForeground }}>
              No {reviewFilter === "correct" ? "correct" : "incorrect"} questions to review.
            </Text>
          </View>
        )}
      </View>
